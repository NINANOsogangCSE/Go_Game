...
Board.draw_board = function() {
	...
	$('#boardContainer').css("margin-top", 0).css("margin-left", (width - Board.board_length) / 2);
...
function stone(S){
	...
	this.x = S.xPosition;
	this.y = S.yPosition;
	this.color = S.piece_color;
	this.num = S.move_number;

}
function submitRecord(){

	$('<input type="hidden" name="record"/>').val(Game.RecString).appendTo('#sendRecord');
...
Board.place_piece = function (move) {
		...
		var piece_string = '<div class="circle" id="piece' + this.piece_number_placed + '" align = "center"><p style="font-size:10px">' + (Game.moveNum + 1) + '</p></div>'
